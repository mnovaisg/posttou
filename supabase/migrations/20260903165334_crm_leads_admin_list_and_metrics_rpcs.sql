-- Base comum: 1 linha por organização (a unidade comercial real —
-- subscriptions é 1:1 com organizations). "Marca" usa a primeira
-- workspace da organização por created_at — suficiente hoje (só o
-- plano Agência permite mais de 1 workspace por organização; caso raro
-- na base atual, documentado como limitação conhecida do v1).
create or replace function public._admin_lead_base()
returns table (
...
as $$
  select
    ...
    s,
    exists(
      select 1 from public.subscription_status_history h
      where h.organization_id = o.id and h.reason = 'user_requested'
    ),
    public._lead_commercial_status(
      s,
      exists(
        select 1 from public.subscription_status_history h
        where h.organization_id = o.id and h.reason = 'user_requested'
  ...
  left join lateral (
    select id from public.workspaces w2 where w2.organization_id = o.id order by w2.created_at asc limit 1
  ) w on true
$$;

revoke execute on function public._admin_lead_base() from public;

-- Listagem paginada server-side (mesmo padrão de admin_list_coupons_system).
...
revoke execute on function public.admin_list_leads_system(text, text, text, billing_interval, timestamptz, timestamptz, text, text, text, boolean, boolean, integer, boolean, integer, integer) from public;
grant execute on function public.admin_list_leads_system(text, text, text, billing_interval, timestamptz, timestamptz, text, text, text, boolean, boolean, integer, boolean, integer, integer) to authenticated;
