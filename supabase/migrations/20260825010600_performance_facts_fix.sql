-- Fase 10 — correção encontrada durante os testes reais: o limite superior
-- do período usava `< v_period_end` com v_period_end = current_date,
-- excluindo publicações de HOJE (a data de hoje nunca é "menor que hoje").
-- Corrigido para `<=`, incluindo o dia corrente no período atual.
