...
create or replace function public.upsert_pilot_settings(
  ...
  p_allowed_formats content_type[],
  ...
  p_temporary_objective_expires_at timestamp with time zone,
  p_default_instagram_account_id uuid,
  p_max_credits_per_window bigint,
  p_format_mix jsonb default null::jsonb,
  p_auto_generate_art boolean default false,
  p_always_require_approval boolean default true
...
begin
  ...
  insert into public.pilot_settings (
    ...
    max_credits_per_window, format_mix, auto_generate_art, always_require_approval
  ) values (
    ...
    p_max_credits_per_window, v_format_mix_normalized, coalesce(p_auto_generate_art, false), coalesce(p_always_require_approval, true)
  )
  on conflict (workspace_id) do update set
    ...
    always_require_approval = excluded.always_require_approval,
    updated_at = now()
