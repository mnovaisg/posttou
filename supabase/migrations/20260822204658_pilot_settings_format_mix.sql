-- Fase 11 — pilot_settings.format_mix: peso ponderado entre formatos
-- permitidos (post/carrossel), separado de allowed_formats (que só decide
-- SE um formato pode ser usado, não QUANTO). Mesmo tratamento de
-- editorial_mix: dica pro planner, normalizada pra ~100% no backend,
-- nunca decidida pela IA. Reel continua bloqueado (sem pipeline real).
alter table public.pilot_settings
  add column format_mix jsonb;

comment on column public.pilot_settings.format_mix is 'Fase 11: peso desejado entre formatos permitidos (ex.: {"post":45,"carrossel":55}). Chaves sempre subconjunto de allowed_formats, valores normalizados a ~100% pelo backend. Null = sem preferência explícita (planner decide livremente dentro de allowed_formats, comportamento pré-Fase 11).';
...
begin
  ...
  -- format_mix (Fase 11): opcional. Quando presente, toda chave precisa
  -- ser um formato permitido e nunca 'reel' — nunca confiamos em nada
  -- vindo do cliente sem revalidar contra allowed_formats aqui.
  ...
  insert into public.pilot_settings (
    ...
    max_credits_per_window, format_mix
  ) values (
    ...
    p_max_credits_per_window, v_format_mix_normalized
  )
...
revoke all on function public.upsert_pilot_settings(uuid, public.pilot_mode, integer, integer, integer[], jsonb, public.content_type[], jsonb, boolean, integer, numeric, text, text, timestamptz, uuid, bigint, jsonb) from public, anon;
grant execute on function public.upsert_pilot_settings(uuid, public.pilot_mode, integer, integer, integer[], jsonb, public.content_type[], jsonb, boolean, integer, numeric, text, text, timestamptz, uuid, bigint, jsonb) to authenticated;

-- A assinatura antiga (16 parâmetros, sem format_mix) precisa ser removida
-- explicitamente — Postgres trata função com parâmetro extra opcional
-- como sobrecarga (overload) distinta, não substituição, e o PostgREST
-- resolveria a chamada ambigamente entre as duas.
drop function if exists public.upsert_pilot_settings(uuid, public.pilot_mode, integer, integer, integer[], jsonb, public.content_type[], jsonb, boolean, integer, numeric, text, text, timestamptz, uuid, bigint);
